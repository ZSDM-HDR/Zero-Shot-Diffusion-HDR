...
class DaCLIP(nn.Module):                    
    ...
    def forward(
        ...
        (description1, description2) = text.chunk(2, dim=-1) if text is not None else (None, None)
        if image is None:
            image_features = None
        else:
            image_features, image_degra_features = self.encode_image(image, control=True, normalize=True)
        text_features1 = self.encode_text(description1, normalize=True) if description1 is not None else None
        text_features2 = self.encode_text(description2, normalize=True) if description2 is not None else None

        return {
            "image_features": image_features,
            "text_features1": text_features1,
